...
class Grillo extends PositionComponent {
  ...
  void render(Canvas canvas) {
    ...
    ..color = Color.fromARGB(255, 142, 5, 5);
    ...
    final Cabeza =
        Rect.fromCenter(center: Offset(size.x*.500, size.y*.780), width: size.x*.5500, height: size.y*.43);
    final Ojo1 =
        Rect.fromCenter(center: Offset(size.x*.390, size.y*.705), width:size.x*.115, height: size.y*.120);
    final Ojo2 =
        Rect.fromCenter(center: Offset(size.x*.610, size.y*.705), width:size.x*.115, height: size.y*.120);

    final Pupila1 =
        Rect.fromCenter(center: Offset(size.x*.390, size.y*.725), width:size.x*.080, height: size.y*.090);
    final Pupila2 =
        Rect.fromCenter(center: Offset(size.x*.610, size.y*.725), width:size.x*.080, height: size.y*.090);

    final cachete1=
    Rect.fromCenter(center: Offset(size.x*.410 ,size.y*.845), width: size.x*.420, height: size.y*.200);
    
    final cachete2=
    Rect.fromCenter(center: Offset(size.x*.590,size.y*.845), width: size.x*.420, height: size.y*.200);
    ...
    canvas.drawRect( Rect.fromCenter(center: Offset(size.x*.5,size.y*.530), width: size.x*.490, height: size.y*.075),colorRojo);
