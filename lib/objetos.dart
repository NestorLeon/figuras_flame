...
class Oso extends PositionComponent{
  ...
  void render(Canvas canvas) {
    ...
    final Cabeza =
        Rect.fromCenter(center: Offset(size.x *.400, size.y *.200), width: size.x *.200, height: size.y *.200);
    final Cuerpo =
        Rect.fromCenter(center: Offset(size.x *.400, size.y *.400), width: size.x *.200, height: size.y *.250);
    final Pata1 =
        Rect.fromCenter(center: Offset(size.x *.300, size.y *.330), width:size.x *.150, height: size.y *.120);
    final Pata2 =
        Rect.fromCenter(center: Offset(size.x *.500, size.y *.330), width:size.x *.150, height: size.y *.120);
    final Pata3 =
        Rect.fromCenter(center: Offset(size.x *.300, size.y *.500), width:size.x *.150, height: size.y *.120);
    final Pata4 =
        Rect.fromCenter(center: Offset(size.x *.500, size.y *.500), width:size.x *.150, height: size.y *.120);
      final Oreja1 =
          Rect.fromCenter(center: Offset(size.x *.300, size.y *.150), width:size.x *.080, height: size.y *.080);
      final Oreja2 =
          Rect.fromCenter(center: Offset(size.x *.500, size.y *.150), width:size.x *.080, height: size.y *.080);
    final Boca =
        Rect.fromCenter(center: Offset(size.x *.400, size.y *.220), width:size.x *.100, height: size.y *.080);
    
    final Huella1 =
        Rect.fromCenter(center: Offset(size.x *.280, size.y *.330), width:size.x *.080, height: size.y *.080);
    final Huella2 =
        Rect.fromCenter(center: Offset(size.x *.520, size.y *.330), width:size.x *.080, height: size.y *.080);
    final Huella3 =
        Rect.fromCenter(center: Offset(size.x *.280, size.y *.500), width:size.x *.080, height: size.y *.080);
    final Huella4 =
        Rect.fromCenter(center: Offset(size.x *.520, size.y *.500), width:size.x *.080, height: size.y *.080);
    
    final Ojo1 =
        Rect.fromCenter(center: Offset(size.x *.375, size.y *.180), width:size.x *.030, height: size.y *.030);
    final Ojo2 =
        Rect.fromCenter(center: Offset(size.x *.425, size.y *.180), width:size.x *.030, height: size.y *.030);

    final Nariz =
        Rect.fromCenter(center: Offset(size.x *.400, size.y *.200), width:size.x *.030, height: size.y *.015);
    ...
    final rect = RRect.fromRectAndRadius(
      Rect.fromCenter(center: Offset(size.x *.300, size.y *.530), width: size.x *.100, height: size.y *.070),
      Radius.circular(20),
    );
...
class Guitarra extends PositionComponent {
  ...
  void render(Canvas canvas) {
    ...
    final Cuerpo1 =
        Rect.fromCenter(center: Offset(size.x *.490, size.y *.280), width: size.x *.240, height: size.y *.240);
    final Cuerpo2 =
        Rect.fromCenter(center: Offset(size.x *.650, size.y *.280), width: size.x *.280, height: size.y *.280);
    final Boca1 =
        Rect.fromCenter(center: Offset(size.x *.520, size.y *.280), width: size.x *.100, height: size.y *.100);
    final Boca2 =
        Rect.fromCenter(center: Offset(size.x *.520, size.y *.280), width: size.x *.120, height: size.y *.120);
